...
game.scale.fullscreenTarget = document.getElementById('game-container');
window.game = game;
...
function applyCrispCanvasFix() {
  ...
  const containerWidth = container
    ? Math.max(container.clientWidth, windowWidth)
    : windowWidth;
  const containerHeight = container
    ? Math.max(container.clientHeight, windowHeight)
    : windowHeight;
...
const MONITOR_INTERVAL = 500; // Check every 500ms instead of every frame

function monitorCanvasResolution() {
  const now = Date.now();
  // Throttle monitoring to reduce lag
  ...
  // Quick check - only fix if actually needed
  const needsSizeFix =
    canvas.style.width !== "100%" || canvas.style.height !== "100%";
  const currentResolution =
    canvas.clientWidth > 0 ? canvas.width / canvas.clientWidth : 1;
  const needsResolutionFix = Math.abs(currentResolution - dpr) > 0.1;

  if (needsResolutionFix || needsSizeFix) {
    ...
    if (game.renderer) {
      if (game.renderer.gl) {
        game.renderer.gl.viewport(
          0,
          0,
          expectedInternalWidth,
          expectedInternalHeight
        );
      }
...
function checkCanvasResolution() {
  const now = Date.now();
  // Check every 200ms (less frequent to reduce lag) and stop after fixes are applied
  if (now - lastCheckTime > 200 && fixAttempts < MAX_FIX_ATTEMPTS) {
    const canvas = game.canvas;
    if (canvas) {
    ...
    // Only call monitorCanvasResolution if we haven't reached max attempts
    if (fixAttempts < MAX_FIX_ATTEMPTS) {
      monitorCanvasResolution();
    }
    lastCheckTime = now;
  }
  // Only continue monitoring if we haven't reached max attempts
  if (fixAttempts < MAX_FIX_ATTEMPTS) {
...
game.events.once("ready", () => {
  ...
  setTimeout(() => {
    ...
    const actualResolution =
      canvas && clientWidth !== "N/A" && clientWidth > 0
    ...
    console.log(
      ...
      config.type === Phaser.AUTO ? "AUTO (2)" : config.type
    );
    ...
    console.log(
      "Canvas Resolution:",
      actualResolution,
      "(actual resolution being used)"
    );
    console.log(
      "Canvas Size:",
      `${canvasWidth}×${canvasHeight} (internal pixels)`
    );
    console.log(
      "Canvas Style:",
      `${canvasStyleWidth} × ${canvasStyleHeight} (CSS style)`
    );
    console.log(
      "Canvas Computed:",
      `${computedWidth} × ${computedHeight} (computed CSS)`
    );
    console.log(
      "Canvas Client:",
      `${clientWidth} × ${clientHeight} (actual display size)`
    );
    console.log(
      "Phaser Game Size:",
      game.scale
    ...
    console.log(
      "Phaser Display Size:",
      game.scale && game.scale.displaySize
        ? `${game.scale.displaySize.width}×${game.scale.displaySize.height}`
        : "N/A"
    );
    ...
    if (
      clientWidth !== "N/A" &&
      Math.abs(clientWidth - window.innerWidth) > 1
    ) {
      console.warn(
        "⚠️ Canvas client width",
        clientWidth,
        "does not match window width",
        window.innerWidth
      );
    }
    if (
      clientHeight !== "N/A" &&
      Math.abs(clientHeight - window.innerHeight) > 1
    ) {
      console.warn(
        "⚠️ Canvas client height",
        clientHeight,
        "does not match window height",
        window.innerHeight
      );
    }
    ...
    if (
      actualResolution !== "N/A" &&
      Math.abs(actualResolution - config.resolution) > 0.1
    ) {
      console.warn(
        "⚠️ Resolution mismatch! Config:",
        config.resolution,
        "Actual:",
        actualResolution
      );
      console.warn(
        "⚠️ Canvas may have been reset by Phaser. Monitoring will fix it."
      );
    }
